...
class NIOSelector extends Thread {
	 ...
	 */
	protected NIOSelector(final NIOConnectionCreator p_connectionCreator) {
		...
		for (int i = 0; i < 10; i++) {
			try {
				...
				m_serverChannel.socket().bind(new InetSocketAddress(Core.getConfiguration().getIntValue(ConfigurationConstants.NETWORK_PORT)));
