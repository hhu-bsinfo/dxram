...
public final class ChunkHandler implements ChunkInterface, MessageReceiver, ConnectionLostListener {
	...
	private MigrationsTree m_migrationsTree;	// CID -> migration backup range
			...
			if (MemoryManager.isResponsible(p_chunkID)) {
				...
				while (null == ret) {
					...
					if (primaryPeer == m_nodeID) {
						// Local get
						ret = MemoryManager.get(p_chunkID);
						if (ret==null) {	// chunk does not exist
							Operation.GET.leave();
							return null;
						}
					} else {
	...
	public boolean migrateRange(final long p_startChunkID, final long p_endChunkID, final short p_target)
		...
		if (NodeID.getRole().equals(Role.SUPERPEER)) {
			...
			if (p_startChunkID <= p_endChunkID) {
				chunkIDs = new long[(int) (p_endChunkID - p_startChunkID)];
				m_migrationLock.lock();
				if (p_target != m_nodeID) {
					// Send chunks to p_target
					chunks = new Chunk[(int) (p_endChunkID - p_startChunkID)];
					iter = p_startChunkID;
		...
		try {
			...
			if (LOG_ACTIVE) {
				for (Chunk chunk : chunks) {
					...
					m_migrationsTree.putObject(chunk.getChunkID(), (byte) m_currentMigrationBackupRange.getRangeID(),
							chunk.getSize());
	...
	private void incomingCommandMessage(final ChunkCommandMessage p_message) {
		...
		if (Core.m_command_listener!=null)
			Core.m_command_listener.processCmd(cmd, false);
		else 
	...
	private String chunkinfo(String p_command) {
		String arguments[];
		short[] backupPeers;
		short primaryPeer;
	...
	private void incomingCommandRequest(final ChunkCommandRequest p_request) {
		...
		// command handled in callback?
			...
			if (Core.m_command_listener!=null) {
				res = Core.m_command_listener.processCmd(cmd, true);
			}
			else {
	...
	public static final class BackupRange {
		...
		public BackupRange(final long p_firstChunkIDORRangeID, final long p_backupPeers) {
			this(p_firstChunkIDORRangeID, new short[] {
