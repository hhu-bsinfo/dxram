	...
	public static long malloc(final int p_size) throws MemoryException {
		...
		try {
			...
			if (ret == -1) {
				...
				ret = segment.malloc(p_size);
				if (ret == -1) {
					System.out.println("Thread: " + threadID + "\tSize: " + p_size + " -> Failed Segment: " + segment.m_status.m_freeSpace);
					printDebugInfos();
	...
	public static final class Segment {
		...
		private long malloc(final int p_size) {
			...
			if (address != 0) {
				...
				System.out.println("Could not find proper block!");
			}
