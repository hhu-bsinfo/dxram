...
abstract public class Cmd {
	...
	public void printHelpMsg() {
		...
		if (lines == null) {
			return; // we should never end up here
		}
		...
		for (int i = 1; i < token.length; i++) {
			...
			if (i >= p_arguments.length) {
				if (token[i].indexOf('[') >= 0) {
					return true; // just an optional missing
				}
			...
			if (token[i].indexOf(',') >= 0) { // parse NID,LID tuple
				...
				if ((subarg == null) || (subarg.length < 2)) {
					...
					if (!parse_token(subtoken[0], subarg[0])) {
						return false;
					}
					if (!parse_token(subtoken[1], subarg[1])) {
						return false;
					}
				}
			}

			if (!parse_token(token[i], p_arguments[i])) {
				return false;
			}
		}
