 ...
 */
public class TestJNIIbnet implements JNIIbnet.Callbacks {
    ...
    private IBBufferPool m_bufferPool;
    ...
    public void run(final String[] p_args) {
        ...
        m_bufferPool = new IBBufferPool(m_inOutBufferSize, 100);
        ...
        while (true) {
            ...
            if (!JNIIbnet.postBuffer(m_remoteNodeId, buffer, buffer.capacity())) {
                System.out.printf("ERROR posting buffer to %X\n", m_remoteNodeId);
                try {
                    Thread.sleep(1000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
    ...
    public void receivedBuffer(short p_sourceNodeId, ByteBuffer p_buffer, int p_length) {
        ...
        if (!JNIIbnet.postFlowControlData(p_sourceNodeId, p_length)) {
            System.out.printf("Sending flow control data to %X failed\n", p_sourceNodeId);
        }
