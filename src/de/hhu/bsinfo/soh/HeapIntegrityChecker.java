...
public final class HeapIntegrityChecker {
    private static Vector<AbstractCheck> ms_checks = new Vector<AbstractCheck>();

    /**
     * Modules for the integrity checker.
     */
    static {
        ms_checks.add(new CheckHeapSizeBounds());
        ms_checks.add(new CheckSegmentCount());
        ms_checks.add(new CheckSegmentBounds());
        ms_checks.add(new CheckSegmentCheckFreeBlockListBounds());
        ms_checks.add(new CheckMemoryBlocksSegmentBounds());
    }
    ...
    public static int check(final HeapWalker.Results p_walkerResults) {
        ...
        for (int i = 0; i < ms_checks.size(); i++) {
            if (!ms_checks.get(i).check(p_walkerResults)) {
                ret = -ms_checks.get(i).getID();
                break;
            }
        }
    ...
    private static class CheckMemoryBlocksSegmentBounds extends AbstractCheck {
        ...
        @Override protected boolean checkInner(final Results p_walkerResults) {
            for (HeapWalker.Segment segment : p_walkerResults.m_segments) {
                for (Entry<Long, MemoryBlock> entry : segment.m_memoryBlocks.entrySet()) {
                    ...
                    if (block.m_startAddress < segment.m_segmentBlock.m_startAddress || block.m_endAddress > segment.m_segmentBlock.m_endAddress) {
    ...
    private static class CheckSegmentCheckFreeBlockListBounds extends AbstractCheck {
        ...
        @Override protected boolean checkInner(final Results p_walkerResults) {
            ...
            for (HeapWalker.Segment segment : p_walkerResults.m_segments) {
                ...
                if (segment.m_segmentBlock.m_sizeFreeBlocksListArea !=
                        segment.m_segmentBlock.m_endAddress - segment.m_segmentBlock.m_startAddressFreeBlocksList) {
