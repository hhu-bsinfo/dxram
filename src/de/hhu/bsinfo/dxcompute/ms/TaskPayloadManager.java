...
public final class TaskPayloadManager {
    ...
    public static TaskPayload createInstance(final short p_typeId, final short p_subtypeId) {
        Class<? extends TaskPayload> clazz = m_registeredTaskClasses.get(((p_typeId & 0xFFFF) << 16) | p_subtypeId);
        if (clazz == null) {
            throw new RuntimeException("Cannot create instance of TaskPayload with id " + p_typeId + "|" + p_subtypeId + ", not registered.");
        }

        try {
            Constructor<? extends TaskPayload> ctor = clazz.getConstructor();
            return ctor.newInstance();
        } catch (final NoSuchMethodException | SecurityException | InstantiationException | IllegalAccessException | IllegalArgumentException
                | InvocationTargetException e) {
            throw new RuntimeException("Cannot create instance of TaskPayload with id " + p_typeId + "|" + p_subtypeId + ": " + e.getMessage());
        }
    }
    ...
    public static TaskPayload createInstance(final short p_typeId, final short p_subtypeId, final Object... p_args) {
        ...
        throw new RuntimeException("Cannot create instance of TaskPayload with id " + p_typeId + "|" + p_subtypeId +
     ...
     *             If the type id and subtype id are already used.
     */
    public static void registerTaskPayloadClass(final short p_typeId, final short p_subtypeId, final Class<? extends TaskPayload> p_class) {
        Class<? extends TaskPayload> clazz = m_registeredTaskClasses.put(((p_typeId & 0xFFFF) << 16) | p_subtypeId, p_class);
        if (clazz != null) {
            throw new KeyAlreadyExistsException("Failed registering " + p_class.getSimpleName() + " for " + p_typeId + "/" + p_subtypeId +
                    " failed, key already used");
        }
    }
}
