...
public final class BFSFrontierBenchmarks extends AbstractMain {
	...
	private static final Argument ARG_THREADS =
			new Argument("threads", "2", true, "Number of threads for the multi threaded section");
	...
	private void main(final int p_itemCount, final int p_threads, final float p_randDistFillRate) {
		...
		if (!doSingleThreaded(p_itemCount, testData, testVector, "SingleThreaded/" + p_itemCount,
	...
	private long executeTestSingleThreaded(final FrontierList p_frontierList, final long[] p_testData,
			final String p_table, final String p_column) {
		...
		System.out.println(
				"Data pushed, ratio added items/total items: " + p_frontierList.size() / (float) p_testData.length);
		...
		long vals = 0;
		{
			...
			stopWatch.stop();
			m_tables.set(p_table + "/popFront", p_frontierList.getClass().getSimpleName(), p_column,
					stopWatch.getTimeStr());
	...
	private long executeTestMultiThreaded(final FrontierList p_frontierList, final int p_threadCount,
			final long[] p_testData, final String p_table, final String p_column) {
		Stopwatch stopWatch = new Stopwatch();
		...
		System.out.println("Pushing back data...");
		{
			...
			for (int i = 0; i < threads.length; i++) {
				...
				System.arraycopy(p_testData, i * (p_testData.length / threads.length), threads[i].m_testData, 0,
						threads[i].m_testData.length);
				threads[i].start();
			}
	...
	private boolean doSingleThreaded(final int p_itemCount, final long[] p_testData, final long p_testVector,
		...
		for (FrontierList frontier : frontiersToTest) {
			...
			if (resVector != p_testVector) {
				System.out.println("ERROR: validation of " + frontier.getClass().getSimpleName() + " failed: "
						+ resVector + " != " + p_testVector);
				return false;
			}
		}
	 ...
	 */
	@SuppressWarnings("unused")
	private boolean doMultiThreaded(final int p_itemCount, final int p_threadCount, final long[] p_testData,
			final long p_testVector, final String p_table, final String p_column) {
		System.out.println("---------------------------------------------------");
