...
public abstract class AbstractLockComponent extends AbstractDXRAMComponent {
	 ...
	 *
	 * @param p_localID       Local ID of the chunk to lock.
	 * @param p_lockingNodeID ID of the node that wants to lock
	 * @param p_writeLock     True to acquire a write lock, false for a read lock.
	 * @param p_timeoutMs     Timeout in ms for the lock operation. -1 for unlimited.
	 ...
	 *
	 * @param p_localID         Local ID of the chunk to lock
	 * @param p_unlockingNodeID ID of the node that wants to unlock the chunk
