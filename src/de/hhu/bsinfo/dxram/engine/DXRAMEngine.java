				...
				if (enabled != null && enabled.booleanValue()) {
					...
					try {
						m_components.put(clazz.getName(),
								(AbstractDXRAMComponent) ctor.newInstance(new Object[] {priorityInit, priorityShutdown}));
	...
	private void bootstrap(final String p_overrideNetworkIP, final String p_overridePort,
		...
		for (String configFile : configurationFiles) {
			...
			if (configLoadSuccessful != 0) {
				System.out.println(
						"[ERR][DXRAMEngine] Loading from configuration file failed: could not find configuraiton file.");
			}
		}
