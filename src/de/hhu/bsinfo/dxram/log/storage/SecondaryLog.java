...
public class SecondaryLog extends AbstractLog {
    ...
    public SecondaryLog(final LogComponent p_logComponent, final SecondaryLogsReorgThread p_reorganizationThread, final short p_nodeID,
            ...
            final boolean p_useChecksum) throws IOException, InterruptedException {
    ...
    @Override public final int appendData(final byte[] p_data, final int p_offset, final int p_length) throws IOException, InterruptedException {
        int length = p_length;
        int logEntrySize;
        int rangeSize = 0;
        ...
        if (length <= 0 || length > m_secondaryLogSize) {
            ...
            while (m_secondaryLogSize - determineLogSize() < length) {
                // #if LOGGER >= WARN
                LOGGER.warn("Secondary log for 0x%X is full. Initializing reorganization and awaiting execution", getNodeID());
                // #endif /* LOGGER >= WARN */
     ...
     *         if the caller was interrupted
     */
    public static Chunk[] recoverBackupRangeFromFile(final String p_fileName, final String p_path, final boolean p_useChecksum, final long p_secondaryLogSize,
                        ...
                        while (readBytes < segmentLength) {
                            ...
                            if (currentVersion.equals(logEntryHeader.getVersion(p_segmentData, readBytes))) {
                                ...
                                if ((currentVersion.getEpoch() & 0x8000) != m_versionsBuffer.getEon()) {
                                    ...
                                    if (m_storesMigrations) {
                                        m_versionsBuffer.put(chunkID, currentVersion.getEpoch() ^ 1 << 15, currentVersion.getVersion());
                                    } else {
                                        m_versionsBuffer.put(ChunkID.getLocalID(chunkID), currentVersion.getEpoch() ^ 1 << 15, currentVersion.getVersion());
                                    }
                                }
                            } else {
