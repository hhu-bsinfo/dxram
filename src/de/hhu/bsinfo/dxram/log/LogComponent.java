...
public class LogComponent extends AbstractDXRAMComponent {
	...
	public Chunk[] recoverBackupRangeFromFile(final String p_fileName, final String p_path) {
		...
		try {
			ret = SecondaryLog.recoverBackupRangeFromFile(p_fileName, p_path, m_useChecksum,
					m_secondaryLogSize.getBytes(),
		...
		try {
			if (p_owner == ChunkID.getCreatorID(p_firstChunkIDOrRangeID)) {
				if (!cat.exists(p_firstChunkIDOrRangeID, (byte) -1)) {
					...
					secLog = new SecondaryLog(this, m_secondaryLogsReorgThread, p_owner,
							ChunkID.getLocalID(p_firstChunkIDOrRangeID), cat.getNewID(false),
							false, m_backupDirectory, m_secondaryLogSize.getBytes(),
					...
					cat.insertRange(p_firstChunkIDOrRangeID, secLog, (int) m_secondaryLogBufferSize.getBytes(),
							(int) m_logSegmentSize.getBytes());
				}
			} else {
				if (!cat.exists(-1, (byte) p_firstChunkIDOrRangeID)) {
					// Create new secondary log for migrations
					secLog = new SecondaryLog(this, m_secondaryLogsReorgThread, p_owner, p_firstChunkIDOrRangeID,
							cat.getNewID(true), true, m_backupDirectory, m_secondaryLogSize.getBytes(),
							(int) m_flashPageSize.getBytes(), (int) m_logSegmentSize.getBytes(),
							m_reorgUtilizationThreshold, m_useChecksum);
					// Insert range in log catalog
					cat.insertRange(p_firstChunkIDOrRangeID, secLog, (int) m_secondaryLogBufferSize.getBytes(),
			...
			try {
				...
				if (rangeID == -1) {
					...
					logEntryHeader = MIGRATION_PRIM_LOG_ENTRY_HEADER.createLogEntryHeader(chunkID, length,
							secLog.getNextVersion(chunkID), rangeID, p_owner);
				}
