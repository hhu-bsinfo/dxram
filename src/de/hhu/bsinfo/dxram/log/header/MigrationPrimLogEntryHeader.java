...
public class MigrationPrimLogEntryHeader extends AbstractLogEntryHeader {
	...
	private static final Logger LOGGER =
			LogManager.getFormatterLogger(MigrationPrimLogEntryHeader.class.getSimpleName());

	// Attributes
	private static short m_maximumSize;
	private static byte m_typeOffset;
	private static byte m_ridOffset;
	private static byte m_srcOffset;
	...
	public MigrationPrimLogEntryHeader() {
		m_maximumSize = (short) (LOG_ENTRY_TYP_SIZE + LOG_ENTRY_RID_SIZE + LOG_ENTRY_SRC_SIZE + MAX_LOG_ENTRY_CID_SIZE
				+ LOG_ENTRY_EPO_SIZE
				+ MAX_LOG_ENTRY_LEN_SIZE + MAX_LOG_ENTRY_VER_SIZE + AbstractLogEntryHeader.getCRCSize());
		m_typeOffset = 0;
		m_ridOffset = LOG_ENTRY_TYP_SIZE;
		m_srcOffset = (byte) (m_ridOffset + LOG_ENTRY_RID_SIZE);
		m_nidOffset = (byte) (m_srcOffset + LOG_ENTRY_SRC_SIZE);
	...
	public byte[] createLogEntryHeader(final long p_chunkID, final int p_size, final Version p_version,
		...
		putRangeID(result, p_rangeID, m_ridOffset);
		putSource(result, p_source, m_srcOffset);
	...
	public byte getRangeID(final byte[] p_buffer, final int p_offset) {
		return p_buffer[p_offset + m_ridOffset];
	}

	@Override
	public short getSource(final byte[] p_buffer, final int p_offset) {
		final int offset = p_offset + m_srcOffset;
