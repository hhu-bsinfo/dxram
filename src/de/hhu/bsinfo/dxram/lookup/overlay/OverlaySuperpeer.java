...
public class OverlaySuperpeer implements MessageReceiver {
	...
	public OverlaySuperpeer(final short p_nodeID, final short p_contactSuperpeer, final int p_initialNumberOfSuperpeers,
			...
			final int p_storageMaxSizeBytes, final AbstractBootComponent p_boot,
		...
		m_metadata = new MetadataHandler(new LookupTree[NodeID.MAX_ID],
				new NameserviceHashTable(1000, 0.9f, m_hashGenerator),
				new SuperpeerStorage(p_storageMaxNumEntries, p_storageMaxSizeBytes, m_hashGenerator),
	 ...
	 *
	 * @param p_firstSuperpeer the first superpeer
	 * @param p_lastSuperpeer  the last superpeer
	 ...
	 *
	 * @param p_missingMetadata the new metadata in a byte array
	 * @lock overlay lock must be write-locked
	 */
	 ...
	 *
	 * @param p_failedNode the failed node's NodeID
	 ...
	 *
	 * @param p_failedNode the failed node's NodeID
	 * @param p_role       the failed node's role
	 *                     =======
				...
				if (backupRanges != null) {
					for (BackupRange backupRange : backupRanges) {
						...
						for (i = 0; i < backupPeers.length; i++) {
							if (backupPeers[i] != 0) {
								// #if LOGGER >= INFO
								LOGGER.info("Initiating recovery of range 0x%X on peer 0x%X",
										firstChunkIDOrRangeID, backupPeers[i]);
								// #endif /* LOGGER >= INFO */
	...
	private boolean createOrJoinSuperpeerOverlay(final short p_contactSuperpeer, final int p_sleepInterval) {
		...
		m_stabilizationThread.setName(
				SuperpeerStabilizationThread.class.getSimpleName() + " for " + LookupComponent.class
						.getSimpleName());
	 ...
	 *
	 * @param p_lastBackupSuperpeer the last backup superpeer
	 * @lock overlay lock must be read-locked
	 */
			...
			if (OverlayHelper.isSuperpeerInRange(joiningNode, m_predecessor, m_nodeID)) {
				...
				try {
					m_network.sendMessage(
							new JoinResponse(p_joinRequest, (short) -1, joiningNodesPredecessor, m_nodeID,
									m_superpeers, peers, metadata));
				} catch (final NetworkException e) {
				...
				if (m_superpeers.isEmpty()) {
					...
					try {
						m_network.sendMessage(new NotifyAboutNewSuccessorMessage(joiningNodesPredecessor,
								m_predecessor));
					} catch (final NetworkException e) {
				...
				try {
					m_network.sendMessage(new JoinResponse(p_joinRequest, superpeer, (short) -1, (short) -1,
							null, null, null));
				} catch (final NetworkException e) {
	...
	private void incomingGetLookupRangeRequest(final GetLookupRangeRequest p_getLookupRangeRequest) {
		...
		// #if LOGGER == TRACE
		LOGGER.trace("Got request: GET_LOOKUP_RANGE_REQUEST 0x%X chunkID: 0x%X",
				p_getLookupRangeRequest.getSource(), chunkID);
		// #endif /* LOGGER == TRACE */
		...
		for (long chunkID : chunkIDs) {
			...
			if (OverlayHelper.isPeerInSuperpeerRange(creator, m_predecessor, m_nodeID)) {
				if (m_metadata.removeChunkIDFromLookupTree(chunkID)) {
					...
					// #endif /* LOGGER >= ERROR */
					try {
						m_network.sendMessage(new RemoveChunkIDsResponse(p_removeChunkIDsRequest,
								new short[] {-1}));
					} catch (final NetworkException e) {
						// Requesting peer is not available anymore, ignore it
					}
	...
	private void incomingGetNameserviceEntriesRequest(
		...
		// #if LOGGER == TRACE
		LOGGER.trace("Got request: GET_NAMESERVICE_ENTRIES from 0x%X",
		...
		try {
			m_network.sendMessage(new GetNameserviceEntriesResponse(p_getNameserviceEntriesRequest,
					m_metadata.getAllNameserviceEntries()));
		} catch (final NetworkException e) {
			// Requesting peer is not available anymore, ignore it
		}
	}
					...
					Runnable task = () -> {
						...
						for (short backupSuperpeer : backupSuperpeers) {
							MigrateRangeRequest request =
									new MigrateRangeRequest(backupSuperpeer, startChunkID, endChunkID, nodeID,
											true);
	...
	private void incomingSetRestorerAfterRecoveryMessage(
		...
		// #if LOGGER == TRACE
		LOGGER.trace("Got request: SET_RESTORER_AFTER_RECOVERY_MESSAGE 0x%X",
	 ...
	 *
	 * @param p_request the BarrierFreeRequest
	 */
	 ...
	 *
	 * @param p_request the GetMetadataSummaryRequest
	 */
	 ...
	 *
	 * @param p_replaceBackupPeerRequest the ReplaceBackupPeerRequest
	 */
	private void
	incomingReplaceBackupPeerRequest(final ReplaceBackupPeerRequest p_replaceBackupPeerRequest) {
		short failedPeer;
		short newBackupPeer;
		long firstChunkIDOrRangeID;
		...
		if (!p_replaceBackupPeerRequest.isBackup()) {
			...
			Runnable task = () -> {
				...
				for (short backupSuperpeer : backupSuperpeers) {
					...
					ReplaceBackupPeerRequest request = new ReplaceBackupPeerRequest(backupSuperpeer,
							firstChunkIDOrRangeID, failedPeer, newBackupPeer, true);
					// send as message, only
					try {
						m_network.sendMessage(request);
					} catch (final NetworkException e) {
			...
			if (p_message.getType() == DXRAMMessageTypes.LOOKUP_MESSAGES_TYPE) {
				switch (p_message.getSubtype()) {
					...
					case LookupMessages.SUBTYPE_GET_CHUNKID_FOR_NAMESERVICE_ENTRY_REQUEST:
						incomingGetChunkIDForNameserviceEntryRequest(
								(GetChunkIDForNameserviceEntryRequest) p_message);
						break;
	...
	private void registerNetworkMessages() {
		...
		m_network
				.registerMessageType(DXRAMMessageTypes.LOOKUP_MESSAGES_TYPE, LookupMessages.SUBTYPE_MIGRATE_REQUEST,
						MigrateRequest.class);
		m_network.registerMessageType(DXRAMMessageTypes.LOOKUP_MESSAGES_TYPE,
				LookupMessages.SUBTYPE_MIGRATE_RESPONSE,
		...
		m_network.registerMessageType(DXRAMMessageTypes.LOOKUP_MESSAGES_TYPE,
				LookupMessages.SUBTYPE_INIT_RANGE_REQUEST,
