...
public class OverlayPeer implements MessageReceiver {
	...
	public boolean barrierFree(final int p_barrierId) {
		...
		if (response.getStatusCode() == -1) {
			// #if LOGGER >= ERROR
			LOGGER.error("Freeing barrier 0x%X on superpeer 0x%X failed: barrier does not exist",
					p_barrierId, responsibleSuperpeer);
			// #endif /* LOGGER >= ERROR */
			return false;
		}
			...
			if (-1 != responsibleSuperpeer) {
				...
				if (response.getStatusCode() != 0) {
					...
					LOGGER.error("Allocating temporary storage on superpeer 0x%X for 0x%X, size %d failed: %d",
							response.getSource(), p_storageId, p_size, response.getStatusCode());
					// #endif /* LOGGER >= ERROR */
					return false;
				} else {
					return true;
