...
public class SimpleLocalLockServiceTest extends AbstractMain {
	...
	public static final Argument ARG_CHUNK_SIZE =
			new Argument("chunkSize", "76", true, "Size of the chunks for allocation");
	public static final Argument ARG_CHUNK_COUNT =
			new Argument("chunkCount", "10", true, "Number of chunks to allocate");
		...
		public void run() {
			for (Chunk chunk : m_chunks) {
				...
				if (err != AbstractLockService.ErrorCode.SUCCESS) {
					System.out.println(
							"[Thread " + currentThread().getId() + "] Locking of chunk " + chunk + " failed: " + err);
					continue;
				}

				int waitMs = (m_random.nextInt(5) + 1) * 100;
				System.out.println("[Thread " + currentThread().getId() + "] Chunk " + chunk + " locked, waiting "
						+ waitMs + " ms...");
				...
				if (err != AbstractLockService.ErrorCode.SUCCESS) {
					System.out.println(
							"[Thread " + currentThread().getId() + "] Unlocking chunk " + chunk + " failed: " + err);
				} else {
