 ...
 */
public class SimpleNameserviceTest extends AbstractMain {
    ...
    @Override protected int main(final ArgumentList p_arguments) {
        ...
        for (Entry<Long, String> entry : verificationMap.entrySet()) {
            ...
            if (chunkID != entry.getKey().longValue()) {
                System.out.println("ChunkID from name service (" + ChunkID.toHexString(entry.getKey()) + ") not matching original chunk ID (" +
                        ChunkID.toHexString(chunkID) + "), for name " + entry.getValue() + ".");
            }
        }
