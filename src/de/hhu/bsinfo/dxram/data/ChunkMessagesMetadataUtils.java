 ...
 * 0: length field flag, 0 = length field = number of chunks in message, 1 = length field indicates size of additional length field in message
 ...
 */
public class ChunkMessagesMetadataUtils {
	 ...
	 * @param p_statusCode Status code to set the number of items for.
	 * @param p_numItems Number of items to send and indicate in the status byte.
	 ...
	 * @param p_statusCode Status code from a message (see class header description for structure).
	 ...
	 * @param p_statusCode Status code to get the information from.
	 * @return Size in bytes of the additional length field in the message payload or 0, if the total number of
	 * 			items is stored with the status code.
	 */
	public static int getSizeOfAdditionalLengthField(final byte p_statusCode) {
		...
		if ((p_statusCode & (1 << BIT_OFFSET_FLAG_LENGTH_FIELD)) > 0) {
			size = (p_statusCode >> BIT_OFFSET_LENGTH_FIELD) & BIT_MASK_LENGTH_FIELD;
			size++; // 0 counts as 1 byte length field
		}
	 ...
	 * @param p_status Status byte of the message.
	 * @param p_buffer Payload buffer.
	 * @param p_numItems Number of items to send with this message.
	 */
	 ...
	 * @param p_status Status byte of the message.
	 * @param p_buffer Payload buffer.
	 * @return Number of items in the message of the provided payload buffer, either taken from the status byte
	 * 			or the buffer (depending on the status flags set).
	 */
	 ...
	 * @param p_statusCode Status code to modify.
	 * @param p_set True to set the read lock flag, false to fully clear the lock acquire flag.
	 ...
	 * @param p_statusCode Status code to modify.
	 * @param p_set True to set the write lock flag, false to fully clear the lock acquire flag.
	 * @return Modified status code.
	 */
	 ...
	 * @param p_statusCode Status code to check.
	 ...
	 * @param p_statusCode Status code to get the flags from.
	...
	public static boolean isReadLockFlagSet(final byte p_statusCode) {
		return ((p_statusCode & (1 << BIT_OFFSET_FLAG_LOCK_ACQUIRE)) > 0) &&
				((p_statusCode & (1 << BIT_OFFSET_FLAG_LOCK_TYPE)) == 0);
	}
