...
public class BackupComponent extends AbstractDXRAMComponent implements EventListener<NodeFailureEvent> {
	...
	private short determineReplacementBackupPeer(final short[] p_currentBackupPeers, final short p_failedPeer,
		...
		while (true) {
			...
			if (currentPeer != -1) {
				// #if LOGGER >= INFO
				m_logger.info(BackupComponent.class, "Determined new backup peer for range "
						+ ChunkID.toHexString(((long) m_nodeID << 48) + p_firstChunkIDOrRangeID)
						+ "to replace " + p_failedPeer + ": "
						+ NodeID.toHexString(currentPeer));
				// #endif /* LOGGER >= INFO */
