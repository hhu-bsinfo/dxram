...
public class TcmdChunkunlock extends AbstractTerminalCommand {
    ...
    public void exec(final TerminalCommandString p_cmd, final TerminalServerStdout p_stdout, final TerminalServiceAccessor p_services) {
        ...
        if (ChunkID.getLocalID(cid) == 0) {
            p_stdout.printlnErr("Locking/Unlocking of index chunk is not allowed");
            return;
        }

        AbstractLockService.ErrorCode err = p_services.getService(AbstractLockService.class).unlock(true, cid);
        if (err != AbstractLockService.ErrorCode.SUCCESS) {
            p_stdout.printflnErr("Error unlocking chunk 0x%X: %s", cid, err);
        } else {
