...
public class NativeMemoryTest extends AbstractMain {
	...
	public static final Argument ARG_JNI_PATH =
			new Argument("jniPath", null, false, "Path to JNI file with native memory implementation");
		...
		System.out.println("Test readValue/writeValue...");
		{
			...
			for (int i = 0; i < 8; i++) {
				...
				if (testValue != testValue2) {
					System.out.println(
							"ERROR: readValue/writeValue test(" + i + "): " + Long.toHexString(testValue) + " != "
									+ Long.toHexString(testValue2));
					return -9;
				}
			}
		}
