 ...
 */
public abstract class IODataStream
{		
	...
	public enum OPERATION_MODE
	{
		 ...
		 */
		READ,
		/** Read/write, includes appending and truncating.
		 */
		WRITE,
	}
	...
	protected OPERATION_MODE m_mode = null;
	
	// --------------------------------------------------------------------------------------
	
	/**
	 * Constructor
	 * @param p_type Type name for the stream.
	 * @param p_address Address/File/Path...
	 */
	public IODataStream(final String p_type, final String p_address)
	{
		m_type = p_type;
		m_address = p_address;
	...
	{
		return m_mode == OPERATION_MODE.WRITE;
	}
	...
	{
		return m_mode == OPERATION_MODE.READ;
	}
	 ...
	 * 
	 * @param p_mode Operation mode for the stream.
	 * @param p_overwrite Overwrite any existing files/parameters appointed by the address.
	 * @return ERROR_CODE Result of the operation.
	 */
	public abstract ERROR_CODE open(final OPERATION_MODE p_mode, boolean p_overwrite);
	 ...
	 *  @return ERROR_CODE Result of the operation.
	 */
	public abstract ERROR_CODE close();
	 ...
	 * 
	 * @param p_data Pre-allocated buffer with a size of at least specified size.
	 * @param p_offset Offset of the array to start reading from.
	 ...
	 */
	public abstract int read(final byte[] p_data, final int p_offset, final int p_size);
	
	/** Same as the other read call, but read size is the size of the array and offset 0.
	 * 
	 * @param p_data Preallocated buffer for reading. 
	 * @return Number of bytes read. Compare to check if you hit eof while reading (less bytes read than specified) or -1 on error.
	 */
	 ...
	 * @param p_offset Offset of the array to start writing to.
	 * @param p_size Number of bytes of the array to be written.
	 * @return Number of bytes written or -1 on error.
	 */
	public abstract int write(final byte[] p_data, final int p_offset, final int p_size);
	
	/** Same as the other write call, but write size is the size of the array and offset 0.
	 * 
	 * @param p_data Preallocated buffer for writing. 
	 * @return Number of bytes written or -1 on error.
	 */
	 ...
	 * 
	 * @param p_pos Absolute position to seek to in bytes.
	 * @return True if successful, false on error.
	 */
	public abstract boolean seek(final long p_pos);
