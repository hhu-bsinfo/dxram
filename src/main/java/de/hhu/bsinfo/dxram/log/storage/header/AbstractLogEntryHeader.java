...
public abstract class AbstractLogEntryHeader {
    ...
    static int getMaxHeaderSize() {
        return LOG_ENTRY_TYP_SIZE + LOG_ENTRY_RID_SIZE + LOG_ENTRY_OWN_SIZE + MAX_LOG_ENTRY_CID_SIZE +
                MAX_LOG_ENTRY_LEN_SIZE + ms_timestampSize + LOG_ENTRY_EPO_SIZE + MAX_LOG_ENTRY_VER_SIZE +
                LOG_ENTRY_CHA_SIZE + ChecksumHandler.getCRCSize();
    }
    ...
    public static int getMaxLogEntrySize() {
        if (ms_segmentSize > 4 * 1024 * 1024) {
            return 2 * 1024 * 1024;
        } else if (ms_segmentSize < 64 * 1024) {
            return 64 * 1024;
        } else {
    ...
    static byte getSizeForLengthField(final int p_length) {
        ...
        if (ret > 3) {
            // Larger chunks are split and chained. The chained links have a size needing 3 bytes to store
            ret = 3;
        }
     ...
     */
    public final int getTimestamp(final ByteBuffer p_buffer, final int p_offset) {
