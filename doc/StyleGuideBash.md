...
Copy/paste [from here](https://github.com/bahamas10/bash-style-guide/blob/
  master/README.md) with modifications.
...
Avoid uppercase variable names to avoid overwriting existing environmental
variables unless there's a good reason to use them. Don't use `let` or
`readonly` to create variables.  `declare` should *only* be used for
associative arrays.  `local` should *always* be used in functions.
